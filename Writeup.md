# Pretending that the LLM is a compiler, because it can

This project explores the question: can a large language model act as an “any-language” compiler, and can a constrained runtime make that output executable in a controlled way?

The github repogpt-pretends-itsa-compiler is an experiment built around that question. It takes source code in a declared language, runs it through LLM-driven stages that validates the program, and then executes the resulting intermediate representation (IR) in a virtual machine based in Python. The goal is not to replace traditional compilers. The goal is to study how far model knowledge and structured orchestration can go when they are embedded inside a disciplined software system.
...
generate a plan for lowering it
compile it into a constrained IR
execute that IR in a runtime with explicit rules
...
the CLI reads the source file and builds the configured model backend
the precompiler checks whether the source plausibly matches the declared language and produces a plan
the compiler lowers the source into a JSON IR
the VM interprets that IR and produces stdout, stderr, and an exit code
...
It is not making the claim that an LLM is a drop-in replacement for a real compiler or a real VM. It is testing whether the knowledge encoded in a model can be harnessed in compiler-like and runtime-like roles when the surrounding system is designed carefully enough.
...
# Conclusion
gpt-pretends-itsa-compiler is best understood as an exploration of LLMs as structured systems components. It uses compiler and VM abstractions to test a practical idea: models know a great deal about programming languages and program behavior, but that knowledge becomes useful only when it is connected to deterministic validation, normalization, and execution. It establishes a controlled environment in which an LLM can serve as a compiler-like translator and a runtime-adjacent reasoning component while the rest of the system enforces the rules. The result is, this project effectively compiles any language that is known to the LLM and facilitate compilation and execution of that program.

# Code
https://github.com/mtshomskyieee/gpt-pretends-itsa-compiler

# Medium
https://medium.com/@mtshomsky/using-an-llm-as-a-compiler-550b4f38b77e
